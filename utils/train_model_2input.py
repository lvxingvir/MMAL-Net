...
from utils.eval_model_mura_2input import eval
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            if set == 'CUB':
                ...
                image, images, labels = data
            image, images, labels = image.cuda(), images.cuda(), labels.float().cuda()
            images = F.interpolate(images,size=[448,448])

            optimizer.zero_grad()

            proposalN_windows_score, proposalN_windows_logits, indices, \
            window_scores, _, raw_logits, local_logits, _ = model(image, images, epoch, i, 'train')
