...
def eval(model, testloader, criterion, status, save_path, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(testloader)):
            ...
            pred = (proposalN_windows_logits.data > 0.5).type(torch.cuda.FloatTensor)  # for mura
            # pred = local_logits.max(1, keepdim=True)[1]
            window_correct += pred.eq(labels.view_as(pred)).sum().item()
    ...
    window_accuracy = window_correct / len(testloader.dataset)


    return raw_loss_avg, windowscls_loss_avg, total_loss_avg, raw_accuracy, local_accuracy, window_accuracy, \
           local_loss_avg