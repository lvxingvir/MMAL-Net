...
import numpy as np
from config_bdpt import coordinates_cat, proposalN, set, vis_num
from utils.cal_iou import calculate_iou
from utils.vis import image_with_boxes

def eval(model, testloader, criterion_bp,criterion_ls, status, save_path, epoch):
    model.eval()
    print('Evaluating')
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(testloader)):
            ...
            raw_loss = criterion_bp(raw_logits, labels_bp)   # for the bdpt
            local_loss = criterion_ls(local_logits, labels)
            windowscls_loss = criterion_ls(proposalN_windows_logits,labels)
