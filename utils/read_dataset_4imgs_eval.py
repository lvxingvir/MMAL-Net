import torch
import os
from datasets import dataset,dataset_4imgs_eval
...
def read_dataset(input_size, batch_size, root, set):
    if set == 'CUB':
        ...
        print('Loading Mura')
        name = 'train'
        trainset = dataset_4imgs_eval.MURA_Dataset_4img(data_dir=root, csv_file='MURA-v1.1/%s.csv' % name,
                                        transform=data_transforms[name])
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=8)
        print('Loading testset')

        name = 'valid'
        testset = dataset_4imgs_eval.MURA_Dataset_4img(data_dir=root, csv_file='MURA-v1.1/%s.csv' % name,
                                       transform=data_transforms[name])
