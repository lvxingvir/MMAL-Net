...
def parse_arg():
    ...
    parser.add_argument('--model_name', help='model name',
                        default={'Elbow':'mura_onlyappm_0309_Elbow',
                                 'Finger':'mura_onlyappm_0309_Finger',
                                 'Forearm':'mura_onlyappm_0309_FOREARM',
                                 'Hand':'mura_onlyappm_0309_HAND',
                                 'Humerus':'mura_onlyappm_0309_HUMERUS',
                                 'Shoulder':'mura_onlyappm_0309_SHOULDER_ag',
                                 'Wrist':'mura_onlyappm_0309_WRIST'}, type=dict)
...
class Eval_mura(object):
    def __init__(self, args):
        ...
        for key in self.model_name:
            self.model_dict[key] = os.path.join(self.pth_dir,self.model_name[key],'best_model.pth')


        os.environ['CUDA_VISIBLE_DEVICES'] = '3'
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = MainNet_2input(proposalN=proposalN, num_classes=1, channels=channels)
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(self.testloader)):
                ...
                if i<10000:
                    ...
                    outputs_logits = self.model(image_val, images_val, epoch, i, 'test', self.device)[-3]
    ...
    def __call__(self, *args, **kwargs):
        ...
        self.df.to_csv('eval_result_raw.csv')

        self.rendering(y,pred,title='Normal Raw')

        self.rendering(st_y, st_pred, title='Study Raw')
